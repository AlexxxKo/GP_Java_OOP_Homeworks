...
import OOP_Homework05.Units.Position;
...
//        System.out.println(PrintInfo(npcIndex));
    ...
    private static String getChar(Position position){
        ...
        for (int i = 0; i < Main.TEAM_SIZE; i++) {
            ...
            {
                if(Main.team1.get(i).getCurrentHp() == 0)
                    str ="|"+AnsiColors.ANSI_RED+Main.team1.get(i).getName().toUpperCase().charAt(0)+AnsiColors.ANSI_RESET;
                else {
                    str ="|"+AnsiColors.ANSI_GREEN+Main.team1.get(i).getName().toUpperCase().charAt(0)+AnsiColors.ANSI_RESET;
                    alive = true;
                }
            }
            if (Main.team2.get(i).getPosition().isEquals(position) && !alive)
            {
                if(Main.team2.get(i).getCurrentHp() == 0)
                    str ="|"+AnsiColors.ANSI_RED+ Main.team2.get(i).getName().toUpperCase().charAt(0)+AnsiColors.ANSI_RESET;
                else str ="|"+AnsiColors.ANSI_BLUE+ Main.team2.get(i).getName().toUpperCase().charAt(0)+AnsiColors.ANSI_RESET;
            }
        }
        return str;
    }
